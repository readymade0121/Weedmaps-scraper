...
for x in product_suburls:
    ...
    row = [brand, dispen, names[i], types[i], description, thc[i], cbd[i], sizes[i], prices[i], images[i]]
    i = i+1
    ...
    with open('product_1.csv', 'a', encoding='utf-8', newline='') as f_object:
        ...
        count1 = count1 + 1
        if count1 == 1:
            product_row.writerow(['brand', 'dispensary', 'name', 'type','description', 'thc','cbd', 'size', 'price', 'image'])
